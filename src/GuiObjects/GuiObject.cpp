...
vector<GuiObject*>GuiObject::gui_ObjectOrderList = vector<GuiObject*>();
...
GuiObject::GuiObject(){
    ...
    gui_Hit=false;
    gui_OverrideStaticDraw=false;
    gui_Selected=false;
    
    gui_TextFont="Arial";

    gui_ObjectOrderList.push_back(this);
}
...
void GuiObject::draw(){
    ...
    for (std::vector<GuiObject*>::reverse_iterator it = gui_ObjectOrderList.rbegin() ; it != gui_ObjectOrderList.rend(); ++it){
        GuiObject *object = (GuiObject*)*it;
        if(object->gui_IsVisible && !object->gui_OverrideStaticDraw)  object->drawObject();
    }
...
void GuiObject::registerForInput(App* app){
    if(!gui_Tuio.isConnected()){
        gui_Tuio.connect();
    }


    gui_CbMouseDown = app->getWindow()->getSignalMouseDown().connect( std::bind( &GuiObject::onMouseBegan, this, std::_1 ) );
	gui_CbMouseDrag = app->getWindow()->getSignalMouseDrag().connect( std::bind( &GuiObject::onMouseMoved, this, std::_1 ) );
    gui_CBMouseEnd  = app->getWindow()->getSignalMouseUp()  .connect( std::bind( &GuiObject::onMouseEnded, this, std::_1 ) );
    
          gui_Tuio.registerTouchesBegan(this,  &GuiObject::touchesBegan);
          gui_Tuio.registerTouchesMoved(this,  &GuiObject::touchesMoved);
          gui_Tuio.registerTouchesEnded(this,  &GuiObject::touchesEnded);
...
void GuiObject::touchesBegan(app::TouchEvent event){
        ...
        for( vector<TouchEvent::Touch>::iterator touchIt = event.getTouches().begin(); touchIt != event.getTouches().end(); ++touchIt ) {
            ...
            if(hasPoint( touchIt->getPos()) &&// does this object contain the touch point
                gui_ObjectOrderList.at(getTopMostObject(touchIt->getPos())) == this //is the topmost object and the current object the same
                ){
                
                    // if this object is accepting touches add the touch to the obj's touch list
                    if(gui_IsAcceptingTouches){
...
int GuiObject::getTopMostObject(Vec2f pos){
    ...
    for(int i =0;i<gui_ObjectOrderList.size();i++){
        if( gui_ObjectOrderList.at(i)->hasPoint(pos)){
            return i;
        }
    }
    return -1;
...
void GuiObject::setText(string s){
    ...
    gui_TextBox = TextBox().alignment( TextBox::CENTER ).font( Font(gui_TextFont,18) ).size( Vec2f(getWidth(), TextBox::GROW) ).text( gui_Text+ countLabel );
...
void GuiObject::updateTextBox(){
    string countLabel = "\n Tchs:" + boost::lexical_cast<string>(currentObjTouches.size());
        gui_TextBox.setText(gui_Text+ countLabel );
